...
function InitialSetup() {
	...
	echo -e "Criando a pasta dos arquivos finais...";
	if [ ! -d raijin/final_builds ]; then
		mkdir -p raijin/final_builds && echo -e "Pasta dos arquivos finais criada com sucesso.\n";
	else
		echo -e "A pasta dos arquivos finais já existe. Nada a fazer.\n";
	fi;
};
...
Modelos suportados (Galaxy Grand Prime):
...
Modelos suportados (Galaxy J5):
	7, j53g - SM-J500H (Global, apenas 3G)
	8, j5lte - SM-J500F/G/M/NO/Y (Global, LTE, sem NFC)
...
Você encontrará seu flashable zip em raijin/final_builds/$SelectedDevice.";
		...
		echo -e "A compilação falhou. Imagem do kernel não encontrada. Verificar se o código-fonte contém erros e tentar novamente.";
		exit 1;
	fi;
};
...
para obter ajuda.";
	...
	*)
		...
		case $1 in
			...
			"7" | "j53g")
				echo -e "Modelo selecionado: SM-J500H\n";
				BuildKernelAndDtb j53g;;
			"8" | "j5lte")
				echo -e "Modelo selecionado: SM-J500F/G/M/NO/Y\n";
				BuildKernelAndDtb j5lte;;
			"9" | "j5nlte")
				echo -e "Modelo selecionado: SM-J500FN\n";
				BuildKernelAndDtb j5nlte;;
			"10" | "j5ltechn")
				echo -e "Modelo selecionado: SM-J5008\n";
				BuildKernelAndDtb j5ltechn;;				
			"a" | "all" | "ba" | "buildall")
				BuildForAll="true";
				echo -e "Compilando para todos os modelos. Isso vai levar bastante tempo.";
