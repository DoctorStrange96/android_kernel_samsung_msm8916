...
function InitialSetup() {
	...
	if [ ! -d raijin_oc/final_builds ]; then
		mkdir -p raijin_oc/final_builds && echo -e "La cartella dei file finali è stata creata con successo.\n";
	else
...
Troverai il tuo \"flashable zip\" nella cartella raijin_oc/final_builds/$SelectedDevice.";
		fi;
	else
...
Ti consiglio di mangiare o bere qualcosa. Questo può richiedere ${bold}MOLTO ${normal}tempo.";
				...
				if [[ "$BuildSuccessful" = "true" ]]; then
					echo -e "Il processo intero è stato finito il `date +"%d %B %Y"` alle `date +"%R GMT%z"`.
Troverai i tuoi \"flashable zip\" nella rispettiva cartella raijin_oc/final_builds per ogni modello.";
				fi;;
			*)
