...
Modelli supportati (Galaxy Grand Prime):
...
Modelli supportati (Galaxy J5):
	7, j53g - SM-J500H (Globale, solo 3G)
	8, j5lte - SM-J500F/G/M/NO/Y (Globale, LTE, senza NFC)
	9, j5nlte - SM-J500FN (Europa, LTE, NFC)
	10, j5ltechn - SM-J5008 (Cina)
...
Opzioni di compilazione:
	nc, noclean - non pulire le sorgenti prima una nuova compilazione ${bold}(sconsigliato!)${normal}
	v, verbose - attiva il modo verboso
EOM
`;
	echo -e "$HelpString";
};
...
function BuildKernelAndDtb() {
	...
	echo -e "La compilazione è iniziata il `date +"%d %B %Y"` alle `date +"%R GMT%z"`.";
	...
	if [ -f out/arch/arm/boot/zImage ]; then
		...
		echo -e "La compilazione è finita il `date +"%d %B %Y"` alle `date +"%R GMT%z"`.";
		# Copy zImage
...
Troverai il tuo \"flashable zip\" nella cartella raijin/final_builds/$SelectedDevice.";
		...
		echo -e "Compilazione fallita. Immagine kernel non trovata. Verificare se le sorgenti contengono degli errori, quindi riprovare.";
		exit 1;
	fi;
};
...
Eseguire \"./$ScriptName help\" o \"./$ScriptName h\" 
per un aiuto.";
		exit 0;;
	*)
		InitialSetup;
		...
		"nc" | "noclean")
			NoClean="true";
			echo -e "Le sorgenti non verranno pulite prima la compilazione. Spero che tu sappia cosa stai facendo!";;
		esac;
		case $3 in
		"nc" | "noclean")
			NoClean="true";
		...
		case $1 in
			...
			"7" | "j53g")
				echo -e "Modello selezionato: SM-J500H\n";
				BuildKernelAndDtb j53g;;
			"8" | "j5lte")
				echo -e "Modello selezionato: SM-J500F/G/M/NO/Y\n";
				BuildKernelAndDtb j5lte;;
			"9" | "j5nlte")
				echo -e "Modello selezionato: SM-J500FN\n";
				BuildKernelAndDtb j5nlte;;
			"10" | "j5ltechn")
				echo -e "Modello selezionato: SM-J5008\n";
				BuildKernelAndDtb j5ltechn;;				
			"a" | "all" | "ba" | "buildall")
				BuildForAll="true";
				echo -e "La compilazione si verrà fatta per tutti i modelli sequenzialmente. Questo richiederà molto tempo.";
